...
static uint64_t TimeCount(void);                                                                         // Get hi-res MONOTONIC time measure in mseconds
static double CurrentTime(void);                                                                         // Get current time measure in milliseconds
...
static void InitTimer(void)
{
    ...
    baseTime = TimeCount();      // Get MONOTONIC clock time offset
    startTime = CurrentTime();   // Get current time
}

// Get hi-res MONOTONIC time measure in seconds
static uint64_t TimeCount(void)
{
    uint64_t value = 0;
...
{
    return (double)(TimeCount() - baseTime)/frequency*1000;
}
